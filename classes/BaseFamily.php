...
class BaseFamily {
	...
	function __construct(){
		...
		$query = "SELECT * FROM `famille`.`membres` ORDER BY `membres`.`order` ASC";
	...
	function getMemberByFirstName($firstname){
		foreach($this->members as $member){
			if($member->firstname == $firstname) return($member);
		}
		return(false);
	}
	function getMemberByfbId($firstname){
		foreach($this->members as $member){
			if($member->fbId == $firstname) return($member);
		}
		return(false);
	}
	function getMemberById($firstname){
		foreach($this->members as $member){
			if($member->id == $firstname) return($member);
		}
		return(false);
	}
	function getMemberByMail($mailAddr){
		foreach($this->members as $member){
			if( $member->email !="" && $member->email == $mailAddr) return($member);
		}
		return(false);
	}
	/**
