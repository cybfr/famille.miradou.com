...
class Family  extends BaseFamily {
	...
	function __construct($currentUser){
		...
		if($result){
			...
			foreach ($this->members as $id=>$member){
				$this->constraints[$id] = array($member->id, $this->members[$member->spouse]->id);
	...
	function checkEndDrawers($drawers){
		if (count($drawers) > CONSTRAINT_SIZE*CONSTRAINT_SIZE) return(FALSE);
	}
	...
	function getGiftDraw(){
		...
		foreach ($this->drawers as $member){
			...
			$this->basket=array_diff($this->basket,array($pulled));
			$gift[$member] = $pulled;
		}
		return($gift);
	}
	...
	function resetGiftDraw(){
		$query = "
				DELETE FROM `famille`.`tirages`
				WHERE `tirages`.`annee` = 2012 AND `tirages`.`user` = '".session_id()."'";
		if( ! mysql_query($query, $this->link)){
			syslog(LOG_ERR, "Error resetting draws, session id =".session_id());
			//		} else {
			//			syslog(LOG_INFO, mysql_affected_rows($this->link)." rows deleted, query was $query");
		}
	}
	function getUserGiftDraw(){
		$unknown = new member(array(id=>'unknown'));
		unset($drawing);
		foreach ($this->getValidGiftDraw() as $drawer => $drawn){
			if($drawer != $this->currentUser->id) $drawing[$drawer]=$unknown->id;
			else $drawing[$drawer] = $drawn;
		}
		return($drawing);
	}
	function storeGiftDraw($drawing){
		foreach ($drawing as $drawer => $drawn){
			$query = "
					...
					;";
//					`user`   = '".session_id()."'
			if(!mysql_query($query, $this->link)) syslog(LOG_ERR, "error updating draw, session id = ".session_id()." : ".mysql_error());
		}
	}
	/**
