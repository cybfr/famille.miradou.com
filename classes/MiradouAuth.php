...
class MiradouAuth{
	...
	function sendPwResetMail($mailToAddr){
		...
		$site_url = "https://famille.miradou.com/resetpw.php";
...
"		Bonjour François-Régis,
		...
		* Vous n’avez pas demandé ce changement ? *
		...
		if(mail($mailToAddr,$subject,$msg_body,$headers)){
			return "Un lien vous a été envoyé pour mettre à jour votre mot de passe. Vérifiez votre mail dans quelques instants";
		}else{
			return " There is some system problem in sending login details to your address. Please contact site-admin.";
		}
		return('msg');
	}
